...
- **Verifying**: `emulator-check.yml` now sources `.github/scripts/emulator-widget.sh`. Stage 1 broadcasts the toggle while locked and asserts nothing starts. Stage 2 does `adb root` (works on `google_apis` images), writes the purchase key into the prefs file, and asserts the broadcast starts `TorchService` and a second broadcast stops it — this is the deterministic end-to-end check and fails the job. Stage 3 drives the Pixel Launcher's widget picker with `uiautomator` to drop the 4×4 widget and screenshots it (`06-widget-placed.png`, `07-widget-tapped.png`); it's best-effort and only warns. `widget-log.txt` and `logcat-widget.txt` (tag `LightOnTorch`) carry the details.
