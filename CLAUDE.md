...
`play-status.yml`, `play-release-name.yml`, and `play-track-clear.yml` all authenticate with `PLAY_SERVICE_ACCOUNT_JSON` and open a Play "edit" because the API requires one. `play-status.yml` never commits and always deletes the edit, so it is read-only. The other two default to `apply=false` (preview only, edit deleted) and commit only when `apply=true` *and* `confirm` repeats the target — the version code for `play-release-name.yml`, the track name for `play-track-clear.yml`.
...
- **`samsung-rtl`** — Samsung Remote Test Lab, for reproducing device-only failures like the splash hang. Note RTL has **no public API**: device control is browser-only and even its adb bridge needs a human to click through, so it cannot be automated in CI. Its value here is *watching the screen* — reading logs over its Remote Debug Bridge is not an option (see the `adb` note under Notes). Figure images were omitted to keep the repo small; each figure's caption and source URL are preserved.
...
- `kShowAdBannerDiagnostics` in `lib/flutter_flow/flutter_flow_ad_banner.dart` draws the load failure (error code, consent state) into the banner slot, because `adb` can't be used to read it — see the `adb` note above. **Set it back to `false` before a Play release**; then a failed banner collapses to zero height instead of showing a black box.
...
- **`adb` is not available — there is no way to read logcat from a device.** Don't propose `adb logcat`, `adb install`, or Samsung RTL's Remote Debug Bridge as a diagnostic step; none of them are a path anyone here can take. This rules out the usual way of reading runtime errors that only surface on a device (AdMob `LoadAdError` codes, plugin warnings, native stack traces), because the app's `print`/`debugPrint` output is only ever visible in logcat.

  So a device-only bug has to be diagnosed **on screen instead of in a log**: put the failure into the widget tree (render the error text where the widget would go, or a debug banner), push it to `claude/**` so `build-apk.yml` produces a sideloadable APK, install that, and read the answer off the phone. Budget an extra build round-trip for this — it is the normal cost of every device-only investigation in this repo, not a workaround for one bug.
